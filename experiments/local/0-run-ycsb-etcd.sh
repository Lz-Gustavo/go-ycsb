...
increaseByTargetThroughput() {
    ...
    # same size arrays
    numOps=(100)
    targetThroughputs=(10)

    for workload in ${workloads[*]}; do
        for (( j = 0; j < ${#targetThroughputs[*]}; j++ )); do
            t=${targetThroughputs[$j]}
            n=${numOps[$j]}

            echo "#${i}-${workload}/${j}: executing $t target throughput"
            ${ycsbPath}/bin/go-ycsb run etcd -P ${ycsbPath}/workloads/${workload} -p target=${t} -p threadcount=${numThreads} -p recordcount=${numDiffKeys} -p operationcount=${n} \
                -p etcd.endpoints=${etcdHostname} -p etcd.commandtimeout=${etcdCommandTimeout} \
